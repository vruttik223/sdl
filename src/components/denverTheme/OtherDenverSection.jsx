...
import { osakaSliderOption } from '../../data/SliderSettings';
  ...
  return (
    <>
      ...
      {dataAPI?.products_list_3?.status &&
        dataAPI?.products_list_3?.product_ids.length > 0 && (
          <ProductWrapper
      ...
      {dataAPI?.news_letter?.status && (
        <NewsLetter dataAPI={dataAPI?.news_letter} />
      )}
    </>
  );
};
